...
macro_rules! default_store_spec {
    ($spec:ident,$status:ident,$name:expr) => {
        impl $crate::k8_types::store::StoreSpec for $spec {
            ...
            fn convert_from_k8(
                k8_obj: $crate::k8_types::K8Obj<Self::K8Spec>,
            ) -> Result<Option<$crate::k8_types::store::MetaItem<Self>>, std::io::Error> {
                let ctx =
                    $crate::k8_types::store::MetaItemContext::default().with_ctx(k8_obj.metadata.clone());
                Ok(Some($crate::k8_types::store::MetaItem::new(
